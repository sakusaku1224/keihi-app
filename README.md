...
## 開発背景
普段の業務で使っている経費申請アプリの操作性が悪く、改善したいと思ったのがきっかけです。
テーブルが画面に収まらず横スクロールが発生する、入力はすべて手打ちでインボイス番号や支払先のミスが起きやすい、申請のステータスが分からないなど——こうした不満から、当初はOCRによるレシート読み取り機能を中心に実装しました。

しかし、実際の業務を振り返ると、課題は「入力の手間」だけではなく、「申請忘れ」「締切超過」「領収書管理」によって立替の回収が遅れてしまうことにあると気付きました。実際に、後で忘れないようにと領収書をスマートフォンで撮影したものの、そのまま申請を忘れてしまう、締日を過ぎると、少額なら申請しなくてもいいと諦めてしまうといった声を聞いていました。こうなると、申請者が精算を受けられないだけでなく、本来その月に計上されるべき経費が翌月へずれる原因にもなります。
...
## こだわりポイント
### レシートBOX — 「撮るだけ」で始められる経費精算
従来の経費精算アプリは「申請を作成 → 明細を入力 → 領収書を添付」という手順が必要で、
忙しいときに後回しにしがちでした。

KeiHiでは「レシートBOX」に領収書を撮影するだけで、OCRが自動で金額・支払先・日付を読み取り保存します。
申請書の作成は後からまとめてでき、BOXから選択するだけでフォームに自動入力（プリフィル）されます。

### ダッシュボード — 未回収金額の可視化
「自分がいくら立て替えているか」を一目で把握できるダッシュボードを設計。
締日までのカウントダウンと合わせて、申請のモチベーションを促します。

### レシートBOXでのOCR読み取り 
画像をだけを持たせるのではなく、OCR読み取りを実装することで、金額をダッシュボードで表示できるようにしました。
...
Rails 7の標準フロントエンド。学習コストと開発速度のバランスを重視。
...
モバイルからの領収書撮影時に、HEIC→JPEG変換とファイルサイズ削減を行うため。

### Devise
Rails向け認証の業界標準gem。ゲストログイン機能の実装も容易であるため。
